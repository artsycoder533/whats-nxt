...
const Cart = () => {
  const { cart, removeFromCart, decrementQuantity, incrementQuantity, resetCart } =
    useCart();
  ...
  const checkout = async () => {
    try {
      const response = await fetch("http://whats-nxt.vercel.app/api/checkout", {
        method: "POST",
        headers: {
    ...
    <section className="flex flex-col justify-center gap-8 max-w-[1400px] w-[90vw] mx-auto my-32">
      ...
      {cart.length === 0 ? (
        <div className="flex flex-col gap-8 items-center">
          ...
          <Link href="/products" className="underline underline-offset-4 text-[#B76E79] flex items-center gap-2">
      ...
      {cart.length > 0
        ...
        : null}
      {cart.length > 0 ?<div className="mt-12 md:self-end">
        <p className="flex items-center justify-between">
          <strong>Subtotal:</strong>
          <span>${subtotal}</span>
        </p>
        <p className="font-extralight">Shipping and taxes calculated at checkout.</p>
        <button
